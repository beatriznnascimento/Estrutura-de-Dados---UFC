...
int Tree::_sum_keys(Node *node) { // TODO
    if(node == nullptr){
        ...
        return (node->key + (_sum_key(node->left)) + (_sum_key(node->right)));
    }
}
...
int Tree::_min_key(Node *node) { // TO DO
     if(node->left != nullptr && node->right != nullptr)
        return std::min(node->key, std::min(_min_key(node->left), _min_key(node->right)));
    else if(node->left != nullptr)
        return std::min(node->key, _min_key(node->left));
    else if(node->right != nullptr)
        return std::min(node->key, _min_key(node->right));
    else
        return node->key;
}

int Tree::total_internal_nodes() { // TO DO
